    ...
    for idx, row in df.iterrows():
        ...
        if combined_text.strip():
            ...
            for i in range(len(words)):
                # Single terms - improved filtering
                word = words[i].strip()
                if (len(word) >= min_length and word not in vos_stopwords and 
                    not word.isdigit() and not word.replace('.', '').isdigit() and
                    word.isalnum() and word):
                    all_terms.append(word)
                    term_frequency[word] += 1
                
                # Two-word phrases - improved filtering
                if i < len(words) - 1:
                    ...
                    if (len(word1) >= 2 and len(word2) >= 2 and
                        ...
                        not word1.isdigit() and not word2.isdigit() and
                        word1.isalnum() and word2.isalnum() and
                        word1 and word2):
                ...
                # Three-word phrases - improved filtering
                if i < len(words) - 2:
                    word1, word2, word3 = words[i].strip(), words[i+1].strip(), words[i+2].strip()
                    if (all(len(w) >= 2 for w in [word1, word2, word3]) and
                        all(w not in vos_stopwords and w for w in [word1, word2, word3]) and
                        all(not w.isdigit() for w in [word1, word2, word3]) and
                        all(w.isalnum() for w in [word1, word2, word3])):
                        
                        phrase = f"{word1} {word2} {word3}"
                        all_terms.append(phrase)
                        term_frequency[phrase] += 1
                
                # Four-word phrases - improved filtering
                if i < len(words) - 3:
                    words_slice = [words[i+j].strip() for j in range(4)]
                    if (all(len(w) >= 2 for w in words_slice) and
                        all(w not in vos_stopwords and w for w in words_slice) and
                        all(not w.isdigit() for w in words_slice) and
                        all(w.isalnum() for w in words_slice)):
...
def save_keyword_list_with_freq(keywords_data, filename):
    ...
    try:
        ...
        print(f"Keyword list with frequencies saved: {filename}")
    except Exception as e:
        print(f"Error saving {filename}: {e}")
...
def main():
    ...
    try:
        ...
        # Save results
        output_dir = r"C:\Users\Lenovo\Desktop\Blockchain & Copyright - Bibliometric Analysis\keywords"
        
        try:
            # Create keywords folder if it doesn't exist
            Path(output_dir).mkdir(exist_ok=True)
            
            save_keyword_list_with_freq(vos_author_keywords, f"{output_dir}\\author_keywords_list.txt")
            
            if 'vos_text_terms' in locals():
                save_keyword_list_with_freq(vos_text_terms, f"{output_dir}\\text_terms_list.txt")
            
            print(f"Files saved to: {output_dir}")
            
        except Exception as e:
            print(f"Error saving to keywords directory: {e}")
            print("Saving to current directory...")
            save_keyword_list_with_freq(vos_author_keywords, "author_keywords_list.txt")
            if 'vos_text_terms' in locals():
                save_keyword_list_with_freq(vos_text_terms, "text_terms_list.txt")
